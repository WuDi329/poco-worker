...
async function testTranscoder() {
  ...
  try {
    // 获取视频信息
    console.log('获取视频信息...');
    const videoInfo = await transcoder.getVideoInfo(testVideoPath);
    console.log('视频信息:');
    ...
    const h265Result = await transcoder.transcode(testVideoPath, outputH265Path, h265Requirements);
    
    if (h265Result) {
      console.log('H.264 -> H.265 转码成功');
      
      // 检查输出文件
      const h265Info = await transcoder.getVideoInfo(outputH265Path);
    ...
    const h264Result = await transcoder.transcode(outputH265Path, outputH264Path, h264Requirements);
    
    if (h264Result) {
      console.log('H.265 -> H.264 转码成功');
      
      // 检查输出文件
      const h264Info = await transcoder.getVideoInfo(outputH264Path);
